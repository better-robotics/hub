...
- **The safety floor** — per-command `pwm` self-expiry watchdog (400 ms default,
  ...
  reason a freshly-joined rover is stopped by default.
...
- The **rover declares a queryable** on `robots/<id>/led`.
...
- A rover on boot/reconnect **declares its subscriber first, then `get`s** the
...
- Preserved: instructor-only engage/clear (app-layer auth, § below); rover refuses
...
The topology is a **star**: rovers and the dashboard all talk *to/through the hub*;
no rover↔rover traffic. This matters because zenoh-pico peer mode does not forward
between peers — but it never needs to here. The hub's *own* publications reach every
connected rover, and rover publications reach the hub's *own* subscriptions; the
...
- Rover session mode (client-to-hub vs. peer-with-hub-listen) is an M2/M3 bench
...
- **Rover session mode** — client-to-hub vs. peer-with-listen; measure reconnect
...
- **`fleet/estop` queryable latch** — verify join-time `get` delivers the engaged
  latch to a rebooting rover before it accepts any drive.
