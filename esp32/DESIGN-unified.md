...
**Status:** proposal (2026-07-09). Decide before tearing out the BLE stack.

Collapse the two ESP32 firmwares — `esp32/` (on-chip hub) and
`better-robotics/robot` (rover) — into **one image** whose role is decided at
boot, and replace **BLE onboarding** with an on-device **captive-portal config
page**. The Pi stays the preferred hub; ESP self-election is a *fallback*, not
the default.
