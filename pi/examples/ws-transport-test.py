...
def test_reject():
    """operator with a wrong password → CONNACK not-authorised, through the WS listener."""
    st = {}
    c = newc("operator", "WRONG")
    ...
    print(f"  reject (wrong operator pw over WS): CONNACK rc={st.get('rc')} -> {'OK' if ok else 'FAIL'}")
    return ok
...
def main():
    ...
    subprocess.run(["mosquitto_passwd", "-b", "-c", passwd, "operator", "change-me"], check=True)
